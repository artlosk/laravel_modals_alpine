...
class PostController extends Controller
{
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                return response()->json([
                    'success' => true,
                    'redirect' => route('backend.posts.index')
                ]);
            }
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Ошибка при создании поста: ' . $e->getMessage()
                ], 500);
            }
    ...
    public function update(PostUpdateRequest $request, Post $post)
    {
        ...
        try {
            ...
            return redirect()->route('backend.posts.index')->with('success', 'Пост успешно обновлен.');
        } catch (\Exception $e) {
            DB::rollBack();
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Ошибка при обновлении поста: ' . $e->getMessage()
                ], 500);
            }
