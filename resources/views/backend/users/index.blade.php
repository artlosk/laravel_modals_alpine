        ...
        <div class="d-flex">
            @can('create-users')
                <a href="{{ route('backend.users.create') }}" class="btn btn-primary btn-sm"
                    onclick="event.preventDefault(); window.dispatchEvent(new CustomEvent('open-modal', { detail: { url: '{{ route('backend.users.create') }}', title: 'Создание пользователя' } }))">
                            ...
                            <th>
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'created_at', 'direction' => request('direction') === 'asc' && request('sort') === 'created_at' ? 'desc' : 'asc']) }}"
                                    ...
                                    @if(request('sort') === 'created_at' || !request('sort'))
                                        <i
                                            class="fas fa-sort-{{ (request('direction') === 'asc' && request('sort') === 'created_at') ? 'up' : 'down' }}"></i>
                                    @else
                                        <i class="fas fa-sort text-muted"></i>
                                    @endif
                                </a>
                                    ...
                                    <div class="d-flex gap-1">
                                        @can('edit-users')
                                            <a href="{{ route('backend.users.edit', $user) }}"
                                                ...
                                                @click.prevent="openModal('{{ route('backend.users.edit', $user) }}', 'Редактирование пользователя')">
